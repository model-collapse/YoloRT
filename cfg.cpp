...
int32_t load_config_from_file(std::string path, AllConfig* cfg) {
    ...
    opt.add_options()("YoloCFG.model_file", po::value(&cfg->YoloCFG.model_file), "path to yolo model file");
    opt.add_options()("YoloCFG.class_thres", po::value(&cfg->YoloCFG.cls_thres), "threshold of class prob");
    opt.add_options()("YoloCFG.nms_thres", po::value(&cfg->YoloCFG.nms_thres), "threshold of non-maximum suppression");
    opt.add_options()("ActCFG.model_file", po::value(&cfg->ActCFG.model_file), "path to action detection model file");
    opt.add_options()("ActCFG.name_file", po::value(&cfg->ActCFG.name_file), "path to action detection name file");
    opt.add_options()("ActCFG.batch_size", po::value(&cfg->ActCFG.batch_size), "action detection batch size");
    opt.add_options()("ActCFG.ext_scale", po::value(&cfg->ActCFG.ext_scale), "action detection bouding box extension scale");
